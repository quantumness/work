...
# For each file, open, copy columns H & I into dataframe and create a name for the model
...
# Extract only the media curves
column.data.media <- column.data.all[grep("1-EXP",column.data.all[,2]),]

# For each media curve, split the fuctional form by the '/' sign, text manipulate the output for adstock and denominator
...
# Create final data set and export to CSV in same folder
...
write.csv(final.data,"PSA Curve Extraction.csv")